 ...
 * 
 * ------------------------------------------------------------------------------
 * 本插件的利用只限于RPG制作大师/RPG Maker的正规用户。
 * 商用、非商用、有偿、无偿、面向一般、面向成人均可使用。
 * 使用时不需要联络或报告。另外，也不需要记载制作者的名字。
 * 导入插件的作品附赠形式以外的再分发，请勿转载。
 * 对于因本插件而产生的任何问题，我们难以承担一切责任。
 * ------------------------------------------------------------------------------
            ...
            for (let index in fileReferences) {
                ...
                if (index == 'Moc') {
                    ...
                    this.add(`${resource.name}/Moc`, url, loadOption, function (resource) {
                        live2dData.Moc = DataManager.Live2DMoc(resource.data);
                        live2dData.loading--;
                        next();
                    });
                    live2dData.loading++;
