 ...
 *
 *@update author Lincoln.Lin Minkin.Lin Janis.Yang Ady.Liang
 *
...
(function () {
	...
	if (Function.prototype.name === undefined) {
		Object.defineProperty(Function.prototype, 'name', {
			get: function () { return this.toString().match(/^\s*function\s*(\S*)\s*\(/)[1]; }
		})
	}
	...
	MONKEY.OSInfo = {
		...
		browser: null,
		offLine: location.href.indexOf("http") == -1 ? true : false,
		...
		updataOsInfo: function () {
			...
			try { this.isSurface = MONKEY.OSInfo.maxTouchPoints > 0 ? true : false } catch (e) { }
	...
	MONKEY.Preload = {
		...
		fileOnload: function () {
			...
			this.tagName == "IMG" ? (self.fileData[this.name]['width'] = this.width, self.fileData[this.name]['height'] = this.height) : '';
	...
	MONKEY.Loader = {
		...
		removeScriptOrCss: function ($name, $type, $callback) {
			var te = ($type == "js") ? "script" : ($type == "css") ? "link" : "none",
				ta = ($type == "js") ? "src" : ($type == "css") ? "href" : "none",
				as = document.getElementsByTagName(te);
			for (var i = as.length; i >= 0; i--) {
				if (as[i] && as[i].getAttribute(ta) != null && as[i].getAttribute(ta).indexOf($name) != -1) {
					as[i].parentNode.removeChild(as[i]);
				}
			}
			$callback != undefined ? $callback() : "";
		},
		setLoadGameJs: function ($arr, $numI, $callback) {
			var $fun = arguments.callee;
			$arr[$numI] == '' ? $numI++ : '';
			MONKEY.Loader.loadScript($arr[$numI] + '?t=' + Date.now(), function () {
				$numI++, $numI == $arr.length ? ($callback ? $callback() : '') : $fun($arr, $numI, $callback);
			});
		},
		getLocationURL: function ($numI, $numJ) {
			var folderArr = window.document.location.pathname.split("/"),
				numI = $numI == undefined ? 2 : $numI,
				numJ = $numJ == undefined ? 3 : $numJ,
				folderName = folderArr[folderArr.length - numI],
				url = '',
				bool = window.self === window.top;
			for (var i = 0, l = folderArr.length - numJ; i < l; i++) url += folderArr[i] + '/';
			return { 'url': url, 'bool': bool, 'callbacks': function () { } }
		},
		setExportImg: function ($obj) {
			$obj === undefined ? $obj = {} : '';
			if (render) var render = modernGame.render;
			var $type = $obj.type === undefined ? 'jpeg' : $obj.type,
				$render = $obj.render === undefined ? (render == undefined ? modernGame.render : render) : $obj.render,
				$x = $obj.x === undefined ? 0 : $obj.x,
				$y = $obj.y === undefined ? 0 : $obj.y,
				$w = $obj.w === undefined ? MONKEY.WorkInfo.pWidth : $obj.w,
				$h = $obj.h === undefined ? MONKEY.WorkInfo.pHeight : $obj.h,
				$name = $obj.name === undefined ? 'img.png' : $obj.name,
				$encoderOptions = $obj.encoderOptions === undefined ? 1 : $obj.encoderOptions;
			$type = $type.toLowerCase().replace(/jpg/i, 'jpeg');
			var $url = 'image/' + ($type.match(/png|jpeg|bmp|gif/)[0]);
			var $imgdata = $render.getCanvasDataURL('image/jpeg', $encoderOptions, $x, $y, $w, $h);
			$imgdata = $imgdata.replace($url, 'image/octet-stream');
	...
	MONKEY.Common = {
		logBool: true,
		log: function () {
			if (MONKEY.Common.logBool) return;
		...
		traceDebug: function () {
			...
			if (!output) {
				...
				closeBtn.innerHTML = "×";
				delBtn.innerHTML = "←";
				btn.appendChild(closeBtn), btn.appendChild(delBtn), btn.appendChild(scaleBtn);
				ele.appendChild(btn);
		...
		getBrowser: function () {
			var OsObject = "";
			if (navigator.userAgent.includes("MSIE") || navigator.userAgent.includes("Media")) {
				OsObject = "IE";
			} else if (navigator.userAgent.includes("Firefox")) {
				OsObject = "Firefox";
			} else if (navigator.userAgent.includes("Chrome")) {
				OsObject = "Chrome";
			} else if (navigator.userAgent.includes("Oprea")) {
				OsObject = "Oprea";
		...
		isPC: function () {
			...
			if (userAgentInfo.indexOf("Macintosh") && MONKEY.OSInfo.maxTouchPoints > 0) {
				flag = false;
			}
			return flag;
		},
	...
	MONKEY.Math = {
		...
		afterRotatePoint: function (originalPoint, source, angle, direction) {
			direction = direction == undefined ? true : direction;
			angle = direction == true ? (angle * Math.PI / 180) : (-1 * angle) * Math.PI / 180;
			var xx = source.x - originalPoint.x;
			var yy = source.y - originalPoint.y;
			var a, r;
			a = Math.atan2(yy, xx);
			a += angle;
			r = Math.sqrt(xx * xx + yy * yy);
			return {
				x: (Math.cos(a) * 1000000) / 1000000 * r + originalPoint.x,
				y: (Math.sin(a) * 1000000) / 1000000 * r + originalPoint.y
			}
		},
		lineSegmentTranslation: function (a, b, c) {
			var r = Math.atan2(b[0] - a[0], b[1] - a[1]) + Math.PI / 2;
			var x = Math.sin(r) * c, y = Math.cos(r) * c;
			var a1 = [a[0] + x, a[1] + y];
			var b1 = [b[0] + x, b[1] + y];
			return { a: a1, b: b1 };
		},
		getPerpendicularFoot: function (pt, begin, end) {
			var array = [], dx = begin[0] - end[0], dy = begin[1] - end[1];
			if (Math.abs(dx) < 0.00000001 && Math.abs(dy) < 0.00000001) {
				array = begin;
				return array;
			}
			var u = (pt[0] - begin[0]) * (begin[0] - end[0]) +
				(pt[1] - begin[1]) * (begin[1] - end[1]);
			u = u / ((dx * dx) + (dy * dy));
			array[0] = begin[0] + u * dx;
			array[1] = begin[1] + u * dy;
			return array;
		},
		lineSegmentsIntr: function (a, b, c, d) {
			var a1 = b[1] - a[1], b1 = a[0] - b[0], c1 = 0 - (b[0] * a[1] - (a[0] * b[1])),
				a2 = d[1] - c[1], b2 = c[0] - d[0], c2 = 0 - (d[0] * c[1] - (c[0] * d[1]));
			var y = (c1 * a2 - c2 * a1) / (b1 * a2 - b2 * a1);
			var x = a1 == 0 ? c[0] : (c1 - b1 * y) / a1;
		...
		div: function (_num1, _num2) {
			...
			const { abs } = Math; // 解构常用方法（可选）

			int1 = Number(num1.toString().replace(".", ""));
			int2 = Number(num2.toString().replace(".", ""));
			returnData = int1 / int2;
			returnData = returnData.toString().indexOf("e") > -1
				? this.setScientificNotationToDecimal(returnData, true)
				: returnData;

			if (abs(de2 - de1) > 0) { // 直接调用 Math.abs
				const decimalDiff = abs(de1 - de2); // 直接调用 Math.abs
				returnData = de1 > de2
					? this.moveDecimalPoint(returnData, decimalDiff, "left")
					: this.moveDecimalPoint(returnData, decimalDiff, "right");
			}
	...
	MONKEY.Tween = {
		...
		getBlackData: function (_now, _dataStr, _boo) {
			...
			if (n == 1) {
				...
				now = boo ? parseInt(now) : parseFloat(now);
				numStr = boo ? parseInt(numStr) : parseFloat(numStr);
				switch (mathStr) {
					case "+": returnStr = now + numStr; break;
					case "-": returnStr = now - numStr; break;
				}
			} else if (n == 0) {
				returnStr = boo ? parseInt(dataStr) : parseFloat(dataStr);
			} else if (n < 0) {
				returnStr = boo ? parseInt(dataStr) : parseFloat(dataStr)
			}
			return returnStr;
	...
	MONKEY.Renderer.prototype = {
		...
		render: function (t) {
			...
			if (MONKEY.box2d) {
				MONKEY.box2d.ll_show();
				MONKEY.box2d.world.Step(1 / 60, 10, 10);
				MONKEY.box2d.world.ClearForces();
			}
			if (this.colorStatus) {
				bufferCtx.save();
			...
			if (MONKEY.box2d) MONKEY.box2d.world.DrawDebugData();
		...
		renderDraw: function (_object) {
			...
			if (_object.box2dBody) {
				_object.updateAccordBox2dBody();
			}
		},
		renderDrawImageObject: function (_object, showImage) {
			...
			if (_object.rotate != 0) {
				bufferCtx.translate(_object.rotatePoint.x * (1 / _object.scale.x), _object.rotatePoint.y * (1 / _object.scale.y));
		...
		renderDrawSameGrapAnimation: function (_animation) {
			if (_animation.visible) {
				...
				var w = 1, h = 1;
				if (_animation.imgdata != '') {
					w = _animation.imgdata[_animation.currentFrame].width !== undefined ? _animation.imgdata[_animation.currentFrame].width : (_animation.imgdata[_animation.currentFrame].w !== undefined ? _animation.imgdata[_animation.currentFrame].w : w);
					h = _animation.imgdata[_animation.currentFrame].height !== undefined ? _animation.imgdata[_animation.currentFrame].height : (_animation.imgdata[_animation.currentFrame].h !== undefined ? _animation.imgdata[_animation.currentFrame].h : h);
				}
				else if (_animation.framedata != '') {
					w = _animation.framedata.frames[_animation.currentFrame].frame.w
					h = _animation.framedata.frames[_animation.currentFrame].frame.h
				}
				else {
					w = !!_animation.special.width ? _animation.special.width : showFrame.width,
				...
				var position = { x: 0, y: 0 };
				var drawPosition = { x: 0, y: 0 };
				if (_animation.imgdata != '') {
					position = {
						x: _animation.imgdata[_animation.currentFrame].x,
						y: _animation.imgdata[_animation.currentFrame].y
					};
					drawPosition = {
						x: _animation.imgdata[_animation.currentFrame].sx,
						y: _animation.imgdata[_animation.currentFrame].sy
					};
				}
				else if (_animation.framedata != '') {
					position = {
						x: _animation.frames[_animation.currentFrame].frame.ox,
						y: _animation.frames[_animation.currentFrame].frame.oy
					};
					drawPosition = {
						x: _animation.framedata.frames[_animation.currentFrame].frame.x,
						y: _animation.framedata.frames[_animation.currentFrame].frame.y,
					};
				}
				else {
					drawPosition = {
		...
		renderDrawText: function (_text) {
			if (_text.visible) {
				...
				var width = !!_text.wordWrap ? _text.wrapWidth : _text.special.width;
				var height = !!_text.special.height ? _text.lineHeight : 18;
				var lineWidth = !!_text.lineWidth ? _text.lineWidth : 1;
				...
				var proto = {
					...
					underlineWidth: _text.underlineWidth,
					underline: _text.underline,
					wrapWidth: _text.wrapWidth
				}
				if (_text.textStr != null) {
					var hiddenHeight = height, hiddenWidth = width;
					if (_text.wordWrap) {
						var count = _text.indent == false ? 0 : 4;
						number = parseInt(_text.wrapWidth / _text.fontSize) * 2;
						var txtLength = _text.textStr.length,
							txtArray = _text.textStr.split("");
						hiddenWidth = _text.wrapWidth;
						var wrapTotal = parseInt(_text.wrapWidth / _text.fontSize);
						var reg = /[\u3002|\uff1f|\uff01|\uff0c|\u3001|\uff1b|\uff1a|\u201c|\u201d|\u2018|\u2019|\uff08|\uff09|\u300a|\u300b|\u3008|\u3009|\u3010|\u3011|\u300e|\u300f|\u300c|\u300d|\ufe43|\ufe44|\u3014|\u3015|\u2026|\u2014|\uff5e|\ufe4f|\uffe5]/;
						for (var i = 0; i < txtArray.length; i++) {
							if ((i + (_text.indent ? 2 : 0)) % wrapTotal == 0 && reg.test(txtArray[i]) && i != 0) {
								txtArray.splice(i - 1, 0, "  ");
							}
						}
		...
		renderDrawImage: function (_img, _parameters) {
			...
			var position = parameters.position !== undefined ? parameters.position : { x: 0, y: 0 },
				...
				filter = parameters.filter !== undefined ? parameters.filter : ''
			if (rotate != 0) {
		...
		renderDrawTextFunc: function (_parameters) {
			...
			var position = parameters.position !== undefined ? parameters.position : { x: 0, y: 0 },
				...
				underlineWidth = parameters.underlineWidth !== undefined ? parameters.underlineWidth : 1,
				underline = parameters.underline !== undefined ? parameters.underline : "none",
				textBaseline = parameters.textBaseline !== undefined ? parameters.textBaseline : "alphabetic";
			if (!!!textStr) {
				bufferCtx.restore();
				return;
			}
			bufferCtx.underline = underline;
			bufferCtx.underlineWidth = underlineWidth;
			...
			if (underline !== "none") {
				var textWidth = bufferCtx.measureText(textStr).width > special.width ? special.width : bufferCtx.measureText(textStr).width;
				var sx = (textAlign == "center" ? textWidth / 2 : textAlign == "right" ? textWidth : 0);
				this.drawLines({
					lineWidth: underlineWidth,
					strokeStyle: fillStyle,
					graphVertexs: [[position.x - sx, position.y + (underline) + 10], [position.x - sx + textWidth, position.y + (underline) + 10]],
				});
			}
			bufferCtx.restore();
		},
		drawLines: function (_parameters) {
			...
			for (var i = 0, len = graphVertexs.length; i < len; i++) {
				if (i > 0) {
					if (graphVertexs[i].length == 2) {
						bufferCtx.lineTo(graphVertexs[i][0] - 0.5, graphVertexs[i][1] - 0.5);
					} else if (graphVertexs[i].length == 4) {
						bufferCtx.quadraticCurveTo(graphVertexs[i][0] - 0.5, graphVertexs[i][1] - 0.5,
							graphVertexs[i][2] - 0.5, graphVertexs[i][3] - 0.5,
							graphVertexs[i - 1][graphVertexs[i - 1].length - 2] - 0.5, graphVertexs[i - 1][graphVertexs[i - 1].length - 1] - 0.5);
					} else {
						bufferCtx.bezierCurveTo(graphVertexs[i][0] - 0.5, graphVertexs[i][1] - 0.5,
							graphVertexs[i][2] - 0.5, graphVertexs[i][3] - 0.5,
							graphVertexs[i][4] - 0.5, graphVertexs[i][5] - 0.5);
					}
				} else {
	...
	MONKEY.Resizer = {
		...
		contralSize: function (obj, fw, fh) {
			fw == 0 ? fw = MONKEY.WorkInfo.pWidth : '';
			fh == 0 ? fh = MONKEY.WorkInfo.pHeight : '';
			...
			if (parent == document.body) {
				if (MONKEY.Common.judgeBrand() == "samsung") {
					if ((document.body) && (document.body.clientWidth)) ww = document.body.clientWidth; else if (window.innerWidth) ww = window.innerWidth;
					if ((document.body) && (document.body.clientHeight)) wh = document.body.clientHeight; else if (window.innerHeight) wh = window.innerHeight;
				} else {
	...
	MONKEY.Pointer.prototype = {
		...
		countRotateMatrix: function (tx, ty, _a) {
			...
			var matrix = [0, 0, 1,
				0, 0, 1,
				0, 0, 1];
			matrix[0] = c, matrix[1] = -s, matrix[2] = (1 - c) * tx + ty * s,
				matrix[3] = s, matrix[4] = c, matrix[5] = (1 - c) * ty - tx * s;
			return matrix;
		},
		...
		shuntEvent: function (e, _eventType, _publishType) {
			...
			if (eventType == "mousedown" && this.eventMouseList.length) this.eventDownList.push(this.eventMouseList[this.eventMouseList.length - 1]);
			if (eventType == "mouseup") {
				if (this.eventDownList.length) {
					if (this.eventMouseList.indexOf(this.eventDownList[this.eventDownList.length - 1]) == -1) this.eventMouseList.pop();
				}
				this.eventDownList = [];
			}
	...
	MONKEY.AudioClass.prototype = {
		...
		createAudio: function (_options) {
			...
			if (_options.name === undefined || _options.url === undefined) {
				console.warn("請指定音頻的 name & url!!!");
				return;
			}
			name = _options.name;
			if (self.isCheck(name)) return;
			var autoPlay = _options.autoPlay !== undefined ? _options.autoPlay : false;
			var loops = _options.loops !== undefined ? _options.loops : 1;
			var volume = _options.volume !== undefined ? _options.volume : self.volume;
			url = _options.url;
			...
			var onloadFunc = _options.onLoad !== undefined ? _options.onLoad : self.staticFunc,
				...
				listenstate = _options.listenstate !== undefined ? _options.listenstate : false;
			var progressOption = {
				audio: n_Audio,
				name: name
			};
			self.audioList.push(name);
			self.audios[name] = n_Audio;
			self.audioAttr[name] = {
				currentLoop: 0,
				playLoop: loops,
				status: "stop",
			...
			n_Audio.addEventListener("progress", function () { self.addListenLoad(progressOption); });
			n_Audio.addEventListener("ended", self.audioPlayEnd);
			n_Audio.src = url;
			self.loading ? n_Audio.load() : "";
			n_Audio.volume = volume;
		},
		setPlayAndPauseAudioAll: function () {
			var self = this;
			if (self.playPause) return;
			self.playPause = true;
			self.setPlayAndPauseAudio(self.audioList);
		},
		setPlayAndPauseAudio: function (audioList) {
			var self = this;
			if (Array.isArray(audioList)) {
				for (var i = 0, l = audioList.length; i < l; i++) {
					(function (i) {
						self.audios[audioList[i]].muted = true;
						self.loading ? '' : self.audios[audioList[i]].load();
						self.play(audioList[i]);
						self.pause(audioList[i]);
						self.audios[audioList[i]].muted = false;
						try { self.audios[audioList[i]].currentTime = 0; } catch (e) { }
					})(i);
				};
			};
		},
		...
		getArrayAllAudioDuration: function (_array) {
			var voiceLen = {};
			for (var i in _array) {
				if (_array[i].type == "audio") {
					var name = _array[i].name;
					var audioDuration = MONKEY.Audio.getAudioDuration(name);
					voiceLen[name] = audioDuration;
				}
			}
			var str = "";
			for (var i in voiceLen) {
				str += '"' + i + '"' + ' : ' + voiceLen[i] + ',\n';
			}
			return str;
		},
		...
		play: function (_name, _options) {
			...
			if (_options !== undefined) {
				var listenstate = _options.listenstate !== undefined ? _options.listenstate : false;
				var endedFunc = _options.ended !== undefined ? _options.ended : this.staticFunc;
				this.audioAttr[_name].endFunc = endedFunc;
				if (_options.ended != undefined) {
					this.audios[_name].addEventListener("ended", self.audioPlayEnd);
				}
				this.audioAttr[_name].listenstate = listenstate;
			}
			this.audioAttr[_name].currentLoop = 0;
			if (this.audios[_name]) this.audios[_name].play();
		},
		pause: function (_name) {
			if (!this.isCheck(_name)) {
				console.error("not find " + _name);
			...
			if (this.audios[_name] && this.audios[_name].paused === false) this.audios[_name].pause();
		},
		pauseAll: function () {
			for (var i = 0; i < this.audioList.length; i++) {
				if (this.audios[this.audioList[i]] && this.audios[this.audioList[i]].paused === false)
		...
		stop: function (_name) {
			...
			if (this.audios[_name] && this.audios[_name].paused === false) this.audios[_name].pause();
			if (this.audioAttr[_name].listenstate) {
				this.audios[_name].removeEventListener("ended", this.audioPlayEnd);
			}
			try { this.audios[_name].currentTime = 0; } catch (e) { }
		},
		stopAll: function () {
			for (var i = 0; i < this.audioList.length; i++) {
				if (this.audioAttr[this.audioList[i]].listenstate) this.audios[this.audioList[i]].removeEventListener("ended", this.audioPlayEnd);
				if (this.audios[this.audioList[i]] && this.audios[this.audioList[i]].paused === false) {
					this.audios[this.audioList[i]].pause();
					try { this.audios[this.audioList[i]].currentTime = 0; } catch (e) { }
				}
			}
		},
	...
	MONKEY.Labeler = {
		clone: function (ele, boo) {
			return ele.cloneNode(boo != undefined ? boo : false);
		},
		...
		createElm: function (_options) {
			...
			for (var i in options) {
				...
				} else if (i == "display" || i == "zIndex" || i == "display") {
		...
		setTransform: function (_obj, _attr) {
			if (_obj !== undefined && _attr !== undefined) {
				...
				_obj.style.msTransform = _attr;
			}
		},
		setOrigin: function (_obj, _attr) {
			if (_obj !== undefined && _attr !== undefined) {
				_obj.style.transformOrigin = _attr;
				_obj.style.mozTransformOrigin = _attr;
				_obj.style.webkitTransformOrigin = _attr;
				_obj.style.oTransformOrigin = _attr;
				_obj.style.msTransformOrigin = _attr;
			}
		},
	...
	MONKEY.Animal = function (_parameters) {
		...
		this.filter = parameters.filter !== undefined ? parameters.filter : "";
	...
	MONKEY.Animal.prototype = Object.assign(MONKEY.EventListener.prototype, {
		...
		setPosition: function (x, y) {
			if (this.box2dBody) {
				this.box2dBody.SetPosition(new MONKEY.box2d.b2Vec2((x + this.parent.position.x + this.rotatePoint.x) / MONKEY.box2d.drawScale, (y + this.parent.position.y + this.rotatePoint.y) / MONKEY.box2d.drawScale));
			} else {
		...
		updateAccordBox2dBody: function () {
			if (this.box2dBody) {
				this.position.x = this.box2dBody.GetPosition().x * MONKEY.box2d.drawScale - this.parent.position.x - this.rotatePoint.x;
				this.position.y = this.box2dBody.GetPosition().y * MONKEY.box2d.drawScale - this.parent.position.y - this.rotatePoint.y;
				this.rotate = this.box2dBody.GetAngle() * 180 / Math.PI;
			}
		},
	...
	MONKEY.IntervalAnimation = function (_parameters) {
		...
		parameters.autoPlay !== undefined ? (!!parameters.autoPlay ? this.pause = false : this.pause = true) : "";
	};
	...
	MONKEY.SameGraphAnimation = function (_parameters) {
		...
		this.playTime = parameters.playTime !== undefined ? parameters.playTime : 1;
		this.currentPlayTime = 0;
		this.imgdata = parameters.imgdata !== undefined ? parameters.imgdata : '';
		this.framedata = parameters.framedata !== undefined ? parameters.framedata : '';
	...
	MONKEY.Text = function (_parameters) {
		...
		this.underline = parameters.underline !== undefined ? parameters.underline : "none",
			this.underlineWidth = parameters.underlineWidth !== undefined ? parameters.underlineWidth : 1,
		...
		this.wrapWidth = parameters.wrapWidth !== undefined ? parameters.wrapWidth : this.special.width;
	...
	MONKEY.Text.prototype = Object.assign(Object.create(MONKEY.Animal.prototype), {
		...
		setShadowColor: function (_color) {
		...
		updata: function () {
			...
			if (!this.wordWrap) {
				this.wrapWidth = this.special.width;
			}
		},
	...
	MONKEY.Input.prototype = Object.assign(Object.create(MONKEY.Text.prototype), MONKEY.EventListener.prototype, {
		...
		sMouseDownFunc: function () {
			...
			if (this.inputToBox)
				this.inputBox.value = this.textStr;
	...
	MONKEY.HtmlText = function (_parameters) {
		...
		this.special.width = parameters.width !== undefined ? parameters.width : 0;
		this.styleIndex = 0;
		this.styles = {};
		this.isDrawFunc = true;
		this.renderDraw = false;
	...
	MONKEY.HtmlText.prototype = Object.assign(Object.create(MONKEY.Text.prototype), {
		...
		draw: function (canvas, ctx, renderer) {
			if (this.visible) {
				...
				var tf = this.TextFormat({ font: this.fontFamily, size: this.fontSize, color: this.fillStyle });
				this._htmlTexts = [];
				...
				var totalWidth = 0;
				var self = this;
				var height = self.lineHeight;
				var repText = self._htmlText.replace(/<[^>]+>/g, "");
				for (var i in self._htmlTexts) {
					if (self._htmlTexts[i].textFormat && height < parseInt(self._htmlTexts[i].textFormat.size)) height = parseInt(self._htmlTexts[i].textFormat.size);
					if (self._htmlTexts[i].textFormat) {
						ctx.font = (!!tf.italic ? "italic " : "") + (!!tf.bold ? "bold " : "") + tf.size + "px " + tf.font;
						totalWidth += ctx.measureText(self._htmlTexts[i].text.toString()).width;
					}
				}
				self.special.width = self.special.width !== 0 ? self.special.width : totalWidth;
				if (totalWidth > self.special.width) {
					ratio = self.special.width / totalWidth;
					totalWidth = self.special.width;
				}
				self.special.height = height;
				for (var i in self._htmlTexts) {
					(function (i) {
						var tf = self._htmlTexts[i].textFormat;
						if (tf) {
							ctx.save();
							var str = self._htmlTexts[i].text.toString();
							ctx.lineWidth = self.lineWidth;
							ctx.textBaseline = self.textBaseline;
							ctx.font = (!!tf.italic ? "italic " : "") + (!!tf.bold ? "bold " : "") + tf.size + "px " + tf.font;
							ctx.fillStyle = tf.color;
							ctx.strokeStyle = self.strokeStyle;
							var textWidth = ctx.measureText(str).width * ratio;
							var sx = self.textAlign == "left" ? 0 : (self.textAlign == "center" ? totalWidth / 2 : totalWidth);
							if (self.fillType == "fillText") {
								ctx.fillText(str, -sx + xlen, height, textWidth);
							} else if (self.fillType == "hollowOut") {
								ctx.strokeText(str, -sx + xlen, height, textWidth);
							} else {
								ctx.strokeText(str, -sx + xlen, height, textWidth);
								ctx.fillText(str, -sx + xlen, height, textWidth);
							}
				...
				this.special.width = self.special.width ? self.special.width : xlen;
				if (this.children.length > 0) {
					for (var i = 0, len = this.children.length; i < len; i++) {
						renderer.renderDraw(this.children[i]);
					}
		...
		TextFormat: function (_parameters) {
			var parameters = _parameters || {};
			var self = {}, s = this;
			self.font = parameters.font !== undefined ? parameters.font : "Arial";
			self.size = parameters.size !== undefined ? parameters.size : 15;
			self.color = parameters.color !== undefined ? parameters.color : "#000000";
			self.bold = parameters.bold !== undefined ? parameters.bold : false;
			self.italic = parameters.italic !== undefined ? parameters.italic : false;
			self.underline = parameters.underline !== undefined ? parameters.underline : false;
			...
			self.clone = function () {
				...
				self.getFontText = function () {
					return (self.italic ? "italic " : "") + (self.bold ? "bold " : "") + self.size + "px " + self.font;
				},
					...
					for (var k in css) {
						switch (k) {
							...
							case "font-size":
								...
								break;
							case "font-style":
								self.italic = (css[k] == "italic");
								break;
							case "font-weight":
								self.bold = (css[k] == "bold");
								break;
							case "text-decoration":
								self.underline = (css[k] == "underline");
								break;
						}
					}
				}
	...
	MONKEY.Select = function (_parameters) {
		...
		this.selectBtn = new MONKEY.RegularPolygonGraphics({
	...
	MONKEY.Graphics = function (_parameters) {
		...
		this.globalCompositeOperation = parameters.globalCompositeOperation !== undefined ? parameters.globalCompositeOperation : 'source-over';
	...
	MONKEY.Graphics.prototype = Object.assign(Object.create(MONKEY.Animal.prototype), {
		...
		getScalePoint: function (_points) {
			return this.getCenterPoint(_points);
		},
		...
		draw: function (canvas, ctx, renderer) {
			if (this.visible) {
				...
				if (this.graphVertexs.length > 0) {
					...
					this.lineCap == 'round' ? this.closeStatus = false : '';
					var proto = {
	...
	MONKEY.RegularPolygonGraphics.prototype = Object.assign(Object.create(MONKEY.Graphics.prototype), {
		...
		setApothem: function (_apothem) {
			this.apothem = _apothem ? _apothem : Math.cos(MONKEY.Math.degToRad(180 / this.edges)) * this.radius;
		},
		getApothem: function () {
			return this.apothem;
		},
		draw: function (canvas, ctx, renderer) {
			if (this.visible) {
				...
				if (this.getApothem() == undefined) this.setApothem();
