class LAppDefine {
    constructor(cvs, dragable, loadCallBack = () => { }) {
        //这里配置canvsa元素的id
        this.CANVAS_ID = cvs.value.id
        this.canvas = cvs.value
        this.loadFuc = loadCallBack
        //是否允许拖拽，默认为true
        ...
        //绑定按钮元素id
        this.BUTTON_ID = "Change"

        this.TEXURE_BUTTON_ID = "texure"
        /**
         *  模型定义
        ...
        this.MODELS = [
            ...
            ["model/shifuku/shifuku.model.json", "model/shifuku2/hijiki.model.json"],
        ...
        this.bindEvents()
        this.loadFuc(this.CANVAS_ID, "model/tororo/tororo.model.json");
    }
    ...
    mouseEvent(e) {
        ...
        if (e.type == "mousedown") {
            ...
            this.mouseOffsetx = e.pageX - this.canvas.offsetLeft;
            this.mouseOffsety = e.pageY - this.canvas.offsetTop;
        } else if (e.type == "mousemove") {
            if (this.isDragging == true) {
                var movex = e.pageX - this.mouseOffsetx;
                var movey = e.pageY - this.mouseOffsety;
                if (movex > window.innerWidth - this.canvas.width)
                    movex = window.innerWidth - this.canvas.width;
                if (movex < 0) movex = 0;
                if (movey > window.innerHeight - this.canvas.height)
                    movey = window.innerHeight - this.canvas.height;
                if (movey < 0) movey = 0;
                if (this.IS_DRAGABLE) {
                    console.log(this.canvas.style)
                    this.canvas.style.left = movex + "px";
                    this.canvas.style.top = movey + "px";
                }
            }
